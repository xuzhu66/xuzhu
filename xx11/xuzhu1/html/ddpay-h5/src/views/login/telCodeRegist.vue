...
.page-con
  ...
  .common-tip(v-if="sendStatus") *验证码已发送至 {{registerForm.account}}
  .page-flow
    .info-con
      .item
        .lable 验证码
...
export default defineComponent({
  ...
  setup() {
    ...
    const { userRegistInfo, registerForm } = storeToRefs(userStore)
    const sendPhoneAction = (param) => userStore.sendPhoneAction(param)
    const sendEmailAction = (param) => userStore.sendEmailAction(param)
    const registerAccountAction = (param) => userStore.registerAccountAction(param)
    ...
    const sendTelMsgFun = () => {
      const param = {
        type: 11,
        tel: registerForm.value.account,
        areaCode: '86'
      }
    ...
    const submitFun = () => {
      ...
      registerForm.value.verifyCode = state.code
      const param = registerForm.value
      param.password = MD5Encrypt(param.password)
      registerAccountAction(registerForm.value).then((res) => {
