...
.page-con
  ...
  .common-title {{type==1?'添加支付宝':'编辑支付宝'}}
  ...
  .page-flow
    .info-con
      ...
      .item
        van-cell-group
          van-field(v-model='acctNo' label='' placeholder='请输入支付宝账号' maxlength=50 border=false label-width="60")
...
export default defineComponent({
  ...
  setup() {
    ...
    const submitAlipayAction = (param) => centerStore.submitAlipayAction(param) // 新增
    const updateAlipayAction = (param) => centerStore.updateAlipayAction(param) // 修改
    ...
    const submitBankFun = async() => {
      ...
      if (state.type === '1') {
        ...
        submitAlipayAction(param).then(res => {
      ...
      if (state.type === '2') {
        ...
        updateAlipayAction(param).then(res => {
          state.requestFlag = true
          instance.proxy.$loading.hide()
          router.go(-1)
        })
    ...
    watchEffect(() => {
      state.success = state.realName && state.acctNo && state.fileList.length
    })
    watchEffect(() => {
