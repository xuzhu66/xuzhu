...
import {getHomeHotGameListApi} from "@/store/template3/apiConfig";

import store from '@/store/Store';
import router from "@/router/Router";

import {getAboutUs, getBannerList, getHotList, getGameList} from "@/store/template3/apiConfig"
    ...
    install(that) {
        store.registerModule(['homeStore'], {
            ...
            actions: {
                ...
                async getPlatInfo({commit, dispatch}, data) {
                    try {
                        ...
                        if (res && res.success) {
                            ...
                            window.win.store.commit('SET_TEAMAGENTENABLED', res.data.teamAgentEnabled); // true表示开启合营代理申请，false表示平台没有开启合营代理申请
                            window.win.store.commit('set_cnzz', res.data.cnzz); // cnzz统计
                            window.win.store.commit('set_enableImgCode', res.data.enableImgCode); // 是否调用图形验证码 res.data.enableImgCode
                ...
                // 首页热门游戏-轮播
                async getHomeHotGameListAction({commit}, data) {
                    try {
                        let res = await getHomeHotGameListApi(data)
                        if (res && res.success) {
                            commit('SET_HOME_HOTGAME_LIST', res.data);
                        } else {
                            window.win.tips.warning(res.msg);
                        }
                        return res
